...
function TodoItem({
	...
	return (
		<label>
			<input type="checkbox" checked={todo.completed} style={{margin: '10px'}} onChange={() => onComplete(todo.id)}
				/>
			<span className={ todo.completed ? 'line-through': 'initial'}>	
				{todoContent}
			</span>
			{todo.completed ? <Button handleClick={() => onDelete(todo.id)}>Delete</Button> : <></>}
		</label>
	)
}
